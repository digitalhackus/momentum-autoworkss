...
export function AddInvoice({ onClose, onSubmit, userRole = "Admin", editInvoice }: AddInvoiceProps) {
  ...
  const filteredQuickSearchCustomers = customers.filter((c) => {
    ...
    const nameMatch = (c.name || "").toLowerCase().startsWith(lowerQuery);
    const phoneDigits = (c.phone || "").replace(/[^\d]/g, "");
    const queryDigits = rawQuery.replace(/[^\d]/g, "");
    const phoneMatch = queryDigits.length > 0 && phoneDigits.startsWith(queryDigits);
    const cachedVehicles = vehiclesByCustomerId[c.id] || [];
    const normalizedPlateQuery = rawQuery.replace(/[\s-]/g, "").toUpperCase();
    const vehicleMatch =
      normalizedPlateQuery.length > 0 &&
      cachedVehicles.some((v) =>
        (v.vehicleNumber || "").replace(/[\s-]/g, "").toUpperCase().startsWith(normalizedPlateQuery)
      );
